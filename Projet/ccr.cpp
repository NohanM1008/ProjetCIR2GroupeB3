...
// Gestion globale du planning des vols pour éviter la saturation
...
void CCR::prendreEnCharge(Avion* avion) {
    ...
    Logger::getInstance().log("CCR", "Prise en charge", "Avion " + avion->getNom());
}
...
void CCR::transfererVersApproche(Avion* avion, APP* appCible) {
    ...
    Logger::getInstance().log("CCR", "Transfert vers APP", "Avion " + avion->getNom());
}
